...
export function saveList(l: TodoList): TodoList {
  getDb()
    .prepare(
      `INSERT INTO todo_lists (id, name, folder_id, sort_order, created_at, updated_at)
        VALUES (@id, @name, @folder_id, @sort_order, @created_at, @updated_at)
        ON CONFLICT(id) DO UPDATE SET
          name = @name, folder_id = @folder_id, sort_order = @sort_order, updated_at = @updated_at`
    )
    .run(serialize(l))
  return l
}
...
export function deleteList(id: string): void {
  ...
  getDb().prepare('UPDATE todo_lists SET deleted_at = ?, updated_at = ? WHERE id = ?').run(date, date, id)
}
