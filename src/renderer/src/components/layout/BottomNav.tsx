...
export default function BottomNav() {
  ...
  return (
    <nav className="lg:hidden flex border-t border-gray-200 dark:border-[var(--border)] bg-[var(--bg-surface)] grain-surface">
...
export function SideNav() {
  ...
  return (
    <nav className="hidden lg:flex flex-col border-r border-gray-200 dark:border-[var(--border)] bg-[var(--bg-surface)] w-16 shrink-0 grain-surface">
      {tabs.map(({ label, icon: Icon, path }) => {
        const active = location.pathname === path
        return (
          <button
